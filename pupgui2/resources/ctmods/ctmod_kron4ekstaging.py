...
CT_DESCRIPTION = {'en': QCoreApplication.instance().translate('ctmod_kron4ekstaging', '''Compatibility tool "Wine" to run Windows games on Linux. Official version from the WineHQ sources, compiled by Kron4ek.''')}
...
class CtInstaller(GEProtonInstaller):
    ...
    def __fetch_github_data(self, tag: str) -> dict:
        ...
        if is_wow64:
            ...
            asset_condition = lambda asset: 'amd64-wow64' in asset.get('name', '') and 'staging' in asset.get('name', '')
        elif is_amd64:
            tag = tag.replace(" (amd64)", "")
            asset_condition = lambda asset: 'amd64' in asset.get('name', '') and 'staging' in asset.get('name', '') and 'wow64' not in asset.get('name', '')
        else:
    ...
    def fetch_releases(self, count: int = 100, page: int = 1) -> list[str]:
        ...
        for release in releases_list:
            ...
            for asset in release.get('assets', []):
                ...
                if 'amd64-wow64' in asset_name and self.release_format in asset_name and 'staging' in asset_name:
                    versions_to_display.append(f"{tag_name} (wow64)")
                elif 'amd64' in asset_name and self.release_format in asset_name and 'staging' in asset_name and 'wow64' not in asset_name:
