...
CT_DESCRIPTION = {'en': QCoreApplication.instance().translate('ctmod_2kron4ekvanilla', '''Compatibility tool "Wine" to run Windows games on Linux. Official version from the WineHQ sources, compiled by Kron4ek.''')}
...
class CtInstaller(GEProtonInstaller):
    ...
    def __fetch_github_data(self, tag: str) -> dict:
        ...
        if is_wow64:
            ...
            tag = tag.replace(" (x86)", "")
            asset_condition = lambda asset: 'x86' in asset.get('name', '') and 'staging' not in asset.get('name', '')
        else:
    ...
    def fetch_releases(self, count: int = 100, page: int = 1) -> list[str]:
        ...
        for release in releases_list:
            ...
            for asset in release.get('assets', []):
                ...
                elif 'x86' in asset_name and self.release_format in asset_name and 'staging' not in asset_name:
